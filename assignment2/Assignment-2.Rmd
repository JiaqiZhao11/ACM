...
df = hgf3_vs_shiftBias(ntrials, biases, nbiases, omega, theta, kappa)
df = data.frame(df)

df %>% ggplot() + geom_point(aes(x = 1: ntrials, y = choice1)) + geom_line(aes(x = 1: ntrials, y = datahgf.mu1hat))
...
datap = list(u = df$datahgf.u, yhat = df$datahgf.yhat, n = nrow(df), prior = 1)
data = list(u = df$datahgf.u, yhat = df$datahgf.yhat, n = nrow(df), prior = 0)
